...
public partial class Manage_I_Content_EditNode : CustomerPageAction
{
    ...
    public string GetFileInfo()
    {
        if (Eval("type").Equals("1"))
            return "<a href='javascript:;' onclick='toggleChild(this,"+Eval("id")+")' ><span class='glyphicon glyphicon-folder-open'></span> " + Eval("name") + "</a>";
        else
            return "<a href='javascript:;' data-pid='"+Eval("pid")+"' onclick=\"setVal(this,'" + Eval("rname") + "')\"><img src='/Images/TreeLineImages/t.gif' /> <span class='glyphicon glyphicon-file'></span> "+Eval("name")+"</a>";
    }
    //过滤掉没有文件的文件夹
    ...
    protected void IsChild(M_UserPromotions ups)
    {
        ...
        int pid = nodeBll.GetNode(ups.NodeID).ParentID;
        if (pid > 0)
        {
