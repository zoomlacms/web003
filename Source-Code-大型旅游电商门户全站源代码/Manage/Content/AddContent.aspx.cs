...
public partial class V_Content_AddContent : CustomerPageAction
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ModelID > 0 && NodeID > 0)
        {
            ...
            Label4.Text = GetAlias("Subtitle", fieldDT) + "：";
            tj_txt.Text = GetAlias("EliteLevel", fieldDT);
            zht_txt.Text = GetAlias("Status", fieldDT);
            gx_time.Text = GetAlias("UpDateTime", fieldDT);
            ...
            if (!IsPostBack)
            {
                ...
                else { SpecInfo_Li.Text = "<div style='margin:5px;' class='btn btn-default disabled'><span class='glyphicon glyphicon-info-sign'></span> " + Resources.L.尚未定义专题 + "</div>"; }
                //智能关键词
                ...
                bread += "<div class='pull-right hidden-xs'> <span> <a href='" + customPath2 + "Content/SchedTask.aspx' title='" + Resources.L.查看计划任务 + "'><span class='glyphicon glyphicon-time' style='color:#28b462;'></span></a>" + GetOpenView() + "<span onclick=\"ShowDiag('EditNode.aspx?NodeID=" + NodeID + "','" + Resources.L.配置本节点 + "');\" class='glyphicon glyphicon-cog' title='" + Resources.L.配置本节点 + "' style='cursor:pointer;margin-left:5px;'></span></span></div>";
                Call.SetBreadCrumb(Master, bread);
            }
        }
        else
        { function.WriteErrMsg(Resources.L.参数错误+"!!"); }
    }
    //节点权限操作
    ...
    public string GetOpenView()
    {
        ...
        outstr = " <a href='/" + strurl + "'  target='_blank' title='"+Resources.L.前台浏览 + "'><span class='glyphicon glyphicon-eye-open'></span></a>";
        return outstr;
    }
