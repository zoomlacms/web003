...
namespace ZoomLa.WebSite.User
{
    public partial class User_Register : System.Web.UI.Page
    {
        ...
        public string ReturnUrl { get { return (Request.QueryString["ReturnUrl"] ?? ""); } }
        //用于绑定微信用户
        ...
        protected override void OnPreInit(EventArgs e)
        {
            #region 用户注册字段解析
            //用户配置中心选择的必填项目
            foreach (string field in SiteConfig.UserConfig.RegFieldsMustFill.Split(','))
            {
            ...
            //用户配置中心选择的选填项目
            foreach (string field in SiteConfig.UserConfig.RegFieldsSelectFill.Split(','))
            {
                if (string.IsNullOrEmpty(field)) continue;
        ...
        public void BtnRegstep1()
        {
            ...
            ReqTxtFaceHeight.Enabled = GetEnableValid("FaceHeight");
            ReqTxtFaceWidth.Enabled = GetEnableValid("FaceWidth");
            ...
            ReqTxtICQ.Enabled = GetEnableValid("ICQ");
            ReqTxtMobile.Enabled = GetEnableValid("Mobile");
            ReqTxtMSN.Enabled = GetEnableValid("MSN");
            ReqTxtQQ.Enabled = GetEnableValid("QQ");
            ReqTxtSign.Enabled = GetEnableValid("Sign");
            ReqTxtTrueName.Enabled = GetEnableValid("TrueName");
            ReqTxtUC.Enabled = GetEnableValid("UC");
            ReqTxtUserFace.Enabled = GetEnableValid("UserFace");
            ReqTxtYahoo.Enabled = GetEnableValid("Yahoo");
        ...
        //获取邮件内容模板标签格式
        public DataTable GetRegEmailDt(string username,string checknum,string checkurl)
        {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            ...
            info.Question = DropDownList1.SelectedValue;
            ...
            info.UserBase.FaceHeight = DataConverter.CLng(TxtFaceHeight.Text.Trim());
            info.UserBase.FaceWidth = DataConverter.CLng(TxtFaceWidth.Text.Trim());
            info.UserFace = TxtUserFace.Text.Trim();;
            info.UserBase.Fax = TxtFax.Text.Trim();
            info.UserBase.HomePage = TxtHomepage.Text.Trim();
            info.UserBase.ICQ = TxtICQ.Text.Trim();
            ...
            info.UserBase.MSN = TxtMSN.Text.Trim();
            ...
            info.UserBase.UC = TxtUC.Text.Trim();
            info.UserBase.UserSex = DataConverter.CBool(DropSex.SelectedValue);
            info.UserBase.Yahoo = TxtYahoo.Text.Trim();
        ...
        private void InitProtocol()
        {
            try
            {
                LitProtocol.Text = FileSystemObject.ReadFile(base.Request.MapPath("~/User/Protocol.txt"));
            }
        ...
        private void ShowModelfield()
        {
            ...
            if (UserModelID != 0)
            {
                string reghtmltxt = fieldBll.InputallHtml(UserModelID, 0, new ModelConfig()
                {
                    Source = ModelConfig.SType.UserRegister
                });
        ...
        public string GetBKImg()
        {
            ...
            else { return "http://code.zoomla.cn/user_login.jpg"; }
        }
    }
}