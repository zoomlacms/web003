...
public partial class test_note : System.Web.UI.Page
{
    ...
    private int SaveBy(string json, int status)
    {
        ...
        dr2["FieldValue"] = string.IsNullOrEmpty(conMod.content) ? "t" : conMod.content;//JsonConvert.SerializeObject(conMod.comlist);
        dr3["FieldValue"] = conMod.pic;
        dr4["FieldValue"] = conMod.sys;
        table.Rows.Add(dr); table.Rows.Add(dr2); table.Rows.Add(dr3); table.Rows.Add(dr4);
        //-------------
        if (model.GeneralID > 0) { ZLLog.L("update:" + model.GeneralID + ":" + model.ItemID); conBll.UpdateContent(table, model); }
        else
        {
            ...
            ZLLog.L("add:" + model.GeneralID + ":" + model.ItemID);
        }
