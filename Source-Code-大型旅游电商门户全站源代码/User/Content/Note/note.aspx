<%@ Page Language="C#" AutoEventWireup="true" CodeFile="note.aspx.cs" Inherits="test_note" MasterPageFile="~/Common/Master/Empty1.master" ValidateRequest="false" ClientIDMode="Static" %>
...
<div class="h5">图片建议选择尺寸大于1920*650的高清大图，如相机原图</div>
</div>
...
<div class="row">
<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12" style="min-height:700px;">
<div class="set_title">
<input type="text" class="form-control" ng-model="comMod.title" placeholder="填写游记标题" maxlength="48" />
</div>

<div class="set_content">
    <asp:TextBox ID="EditorContent" runat="server" TextMode="MultiLine" Width="100%" Height="700px" ClientIDMode="Static"></asp:TextBox>
    <script type="text/javascript">
        UE.getEditor('EditorContent', { toolbars: [['FullScreen', 'Undo', 'Redo','removeformat','autotypeset','paragraph', 'fontfamily', 'fontsize', 'Bold', 'Italic', 'NumberedList', 'BulletedList', 'Smiley', 'ShowBlocks', 'Maximize', 'underline', 'fontborder', 'strikethrough', '|', 'justifyleft', 'justifycenter', 'justifyright', 'justifyjustify', 'simpleupload', 'insertimage', 'insertvideo', 'emotion', 'attachment', 'map']] });
    </script>
    <div>
...
<div class="con-item" ng-repeat="item in comMod.comlist track by $index|orderBy: '-orderID'" ng-switch="item.type" hidden>
<div class="toolbar">
...
<div class="view_bottom" style="left:0">
...
<button type="button" class="btn btn-default" onclick="note.view();">预览</button>
...
<div class="note_drafts_t">草稿箱（4）<span class="pull-right"><a href="#">写游记帮助</a></span></div>
<div class="note_drafts_c">
<ul>
<%Call.Label("{ZL.Label id=\"获取用户游记草稿列表\" ShowNum=\"10\" TitleNum=\"18\" /}");%>
</ul>
</div>
<div class="note_drafts_b">
<a href="#"><i class="fa fa-angle-up"></i></a>
</div>
</div>
<div class="note_ad">
<a href="#"><img src="/Template/Travel/style/images/acerthree1.jpg" alt="" /></a>
</div>
</div>
</div>
...
<asp:Button runat="server" ID="View_Btn" OnClick="View_Btn_Click" />
...
#OAkeyword{ margin-top:10px; height:36px; border:1px solid #ccc; padding-top:3px; background:url(res/inp_bg.gif) no-repeat right center}
...
<script>
    ...
    .controller("NoteCtrl", function ($scope, $compile) {
        scope = $scope;
        scope.note = note;
        $scope.comMod = { id: 0, topimg: "", pic: "", title: "", mp3: "", comlist: [], sys: "", content: "" };
    ...
    note.autoSave = function () {
        ...
        setInterval(function () {
            ...
            $.post("", { "action": "save", value: $("#Save_Hid").val() }, function (data) {
                ...
                console.log("autoSave", data, scope.comMod.id);
            });
        }, times);
    }
    note.preSave = function () {
        ...
        $div.append(html);
        scope.comMod.pic = $div.find("img:first").attr("src");
        if (scope.comMod.pic) { scope.comMod.pic = scope.comMod.pic.replace(uploadfiles, ''); }
        //var sysstr=GetFirstText();
        var sys = ue.getContentTxt();
        if (sys.length > 200)
            sys = sys.substring(0, 200);
        scope.comMod.sys = sys;
        scope.comMod.content = ue.getContent();
        $("#Save_Hid").val(angular.toJson(scope.comMod));
        return true;
    }
    note.view = function () { note.preSave(); $("#View_Btn").click(); }
    note.save = function () { note.preSave(); $("#Save_Btn").click(); }
    ...
    /*-------TopImg-------*/
    function selTopimg() { $("#topimg_file").val(""); $("#topimg_file").click(); }
    function setTopimg() {
	...
	$().ready(function () {
	    $("#OAkeyword").tabControl({ maxTabCount: 5, tabW: 80 }, $("#Keywords").val());
	})
