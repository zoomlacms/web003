<%@ Page Language="C#" AutoEventWireup="true" CodeFile="BKEditor.aspx.cs" Inherits="Guest_Baike_BKEditor" MasterPageFile="~/Common/Master/Empty.master" ValidateRequest="false" %>
...
<h2>概述</h2>
<input type="file" id="pic_up" style="display: none;" onchange="pic.upload();" />
</div>
...
/*-------------------------------------------*/
#content_wrapper {}
.header {position:fixed;height:87px;background-color:#F5F6F8;border-bottom:1px solid #cfd0d1; text-shadow:none;width:100%;z-index:9999;top:0px;}
...
$(function () {
	...
	//});
	{
		...
		if (!ZL_Regex.isEmpty(val)) { info.data = JSON.parse(val); info.dataToEdit(); }
			...
			info.addRow({ name: "景区名称" });
			info.addRow({ name: "门票价格" });
			info.addRow({ name: "地理位置" });
			info.addRow({ name: "景点级别" });
			info.addRow({ name: "景点类型" });
			info.addRow({ name: "最佳游玩季节" });
			info.addRow({ name: "美食特产" });
			info.addRow({ name: "建议游玩时长" });
			info.addRow({ name: "相关人物" });
			info.addRow({ name: "适宜游玩方式" });
			info.addRow({ name: "出游建议" });
			info.addRow({ name: "开放时间" });
		}
	}
	//---------------------------------------
	var $pic = $("#pic_img");
